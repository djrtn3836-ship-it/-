...
class DailyReportGenerator:
    ...
    async def generate_and_send(self):
        ...
        try:
            ...
            lines = []
            lines.append(f"<b>🏛️ [QUANT DESK] 데일리 전략 브리프 v6.1</b>")
    ...
    def _get_daily_risks(self, regime: str, decisions: List[Dict]) -> List[str]:
        base = ["① 전력 인허가 지연 → AI 인프라 상단 제약", "② HBM 가격 피크아웃 조기화 가능성", "③ 로봇 파일럿→반복 수주 전환 증거 부재"]
        if "상승" in regime:
            return ["① 과열된 로봇 섹터 밸류에이션 조정 리스크", "② NVIDIA 가이던스 하향 시 반도체 전체 멀티플 압축", "③ 외국인 수급 급변 시 변동성 확대"]
        elif "약세" in regime:
            return ["① 추가 하방 시 숏스퀴즈 가능성 (반등)", "② 환율 급등 시 외국인 자금 이탈 가속화", "③ 지정학적 리스크 재부각 (대만·반도체)"]
        return base

    def _get_action_items(self, regime: str, decisions: List[Dict]) -> List[str]:
        if "상승" in regime:
            return ["✅ Core(반도체) 비중 유지, 추세 추종", "✅ Tactical(로봇)은 조정 시 분할 매수", "✅ Optionality(양자)는 5% 이하로 유지"]
        elif "약세" in regime:
            return ["⚠️ Core 비중을 50%로 축소 (방어)", "⚠️ 현금 비중 20% 이상 확보", "⚠️ 로봇·양자 비중 긴급 점검"]
        else:
            return ["🔍 Core 유지 (60%), Tactical 20%, 현금 15%", "🔍 외국인 수급·TSMC 발언 모니터링 강화", "🔍 로봇 이벤트(실적발표) 전까지 대기"]