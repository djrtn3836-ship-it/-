...
class ProjectFileScanner:
    def __init__(self):
        ...
        self.results = {"total_files": 0, "passed": 0, "syntax_errors": 0, "import_errors": 0, "pyflakes_errors": 0, "other_errors": 0, "details": []}
    ...
    def should_scan(self, p: Path) -> bool:
        for part in p.parent.parts:
            if part in self.exclude_dirs: return False
        return p.suffix == '.py'
    ...
    def scan_file(self, p: Path):
        if not self.should_scan(p): return
        self.results["total_files"] += 1
        mod = self.get_module_name(p)
        status, msg, err_type = "PASS", "", None
        has_err = False
        try:
            with open(p, 'r', encoding='utf-8') as f: source = f.read()
            ast.parse(source)
        ...
        if not has_err and HAS_PYFLAKES:
            try:
                with open(p, 'r', encoding='utf-8') as f: source = f.read()
                errors = []
                def reporter(m): errors.append(m)
                pyflakes.api.check(source, str(p), reporter)
                if errors:
                    status, err_type = "FAIL", "Pyflakes"
                    msg = str(errors[0]).split('\n')[0]
                    for e in errors:
                        self.results["details"].append({"file": str(p.relative_to(self.root_dir)), "module": mod, "status": "FAIL", "type": "Pyflakes", "message": str(e)})
                    self.results["pyflakes_errors"] += 1
                    has_err = True
            except: pass
        if has_err:
            self.results["details"].append({"file": str(p.relative_to(self.root_dir)), "module": mod, "status": status, "type": err_type, "message": msg})
            if "Syntax" in err_type: self.results["syntax_errors"] += 1; print_fail(f"{p.name} (문법): {msg}")
            elif "Import" in err_type: self.results["import_errors"] += 1; print_fail(f"{p.name} (임포트): {msg}")
            elif "Pyflakes" in err_type: pass
            else: self.results["other_errors"] += 1; print_fail(f"{p.name} (기타): {msg}")
        else:
            self.results["passed"] += 1; print_ok(f"{p.name} ({mod})")

    def run(self):
        print_title("전체 파일 검사 시작")
        print(f"기준: {self.root_dir}")
        if not HAS_PYFLAKES: print_warn("pyflakes 미설치 -> 일부 검사 생략")
        print("-" * 60)
        py_files = list(self.root_dir.rglob("*.py"))
        targets = [f for f in py_files if self.should_scan(f)]
        print_info(f"대상 파일: {len(targets)}개")
        for f in targets: self.scan_file(f)
        print("\n" + "=" * 60)
        ...
        if HAS_PYFLAKES: print(f"  {Colors.RED}[FAIL] Pyflakes: {self.results['pyflakes_errors']}{Colors.END}")
        ...
        try:
            ...
            with open(report_path, 'w', encoding='utf-8') as f: json.dump(self.results, f, ensure_ascii=False, indent=2)
